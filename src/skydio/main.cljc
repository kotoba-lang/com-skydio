(ns skydio.main
  "Kotodama WASM entrypoint for the Skydio clean-room actor (L4) — Clojure port.
  ...
  facts (`skydio.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "skydio")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "skydio_dro"
   ...
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "skydio_tel"
   ...
   {:entity "Payload"    :plural "payloads"    :id-prefix "skydio_pay"
    :fields [:droneId :type :weightKg]        :required [:type :weightKg]
    :coerce {:weightKg :float}       :refs {:droneId "Drone"}}
   {:entity "Swarm"      :plural "swarms"      :id-prefix "skydio_swa"
...
(defn emit-facts
  "EAVT facts for one record: {\"skydio.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "skydio-compat" :tier tier :entities entities} 200])
...
(defn start! [] :skydio-compat/ready)
